...
/*
    NOTE: THIS FILE HAS BEEN MODIFIED FOR SUITABILITY IN THE HURTIGRUTE TIMELAPSE PROJECT.
    IT IS NOT THE ORIGINAL FILE PROVIDED BY THE AUTHOR
*/
...
double MovieDecoder::getDuration()
{
    if (m_pFormatContext)
    {
        return (long double)(m_pFormatContext->duration) / (long double)(AV_TIME_BASE);
    }
...
void MovieDecoder::seek_ex(int64_t timestamp)
{
    if (!m_AllowSeek)
    {
        return;
