...
PngWriter::PngWriter(const string& outputFile) : m_FilePtr(NULL), m_PngPtr(NULL), m_InfoPtr(NULL)
{
    init();
	//m_FilePtr = outputFile == "-" ? stdout : fopen(outputFile.c_str(), "wb");
    fopen_s(&m_FilePtr, outputFile.c_str(), "wb");
...
{
    init();
    png_set_write_fn(m_PngPtr, (void*) &outputBuffer, writeDataCallback, NULL);
}
