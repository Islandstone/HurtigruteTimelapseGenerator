...
**/ 
bool Thread::wait(const wchar_t* m,long ms) {
...
**/ 
void Thread::release(const wchar_t* m) {
	HANDLE h = OpenMutex(MUTEX_ALL_ACCESS,FALSE,m);
	if(h == NULL) {
...
/** create(const char* nm)
  ...
  * creates a Mutex object identified by "nm"
**/  
void Mutex::create(const wchar_t* nm) {
	if(m_hMutex != NULL) {
